...
2. **Crea directorios de trabajo** (se pueden versionar o ignorar según tu flujo):

   ```bash
   mkdir -p models thumbs qdrant_storage logs state
   ```

3. **Levanta servicios (API + Qdrant)**:
...
docker compose run --rm   api   python3 -m app.ingest   --path "/hosthome/Documentos/fotos_f"   --batch 256
```
...
1. Abre `http://localhost:9100/`.
2. Sube una foto con un rostro (idealmente frontal).
3. El sistema:
   - Detecta el rostro.
   - Aplica TTA y genera un embedding robusto.
   - Consulta Qdrant para obtener el top-K (por defecto, 10) más similares.
4. La página muestra, por cada coincidencia:
...
- [InsightFace](https://github.com/deepinsight/insightface) — modelos de reconocimiento facial.
...
Revisa las licencias de estos proyectos antes de utilizar Oceano en entornos productivos.
